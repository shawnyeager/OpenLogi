---
paths:
  - "crates/openlogi-ui/locales/**"
  - "crates/openlogi-ui/src/catalog_parity.rs"
  - "crates/openlogi-core/src/locale.rs"
  - "crates/openlogi-agent/src/main.rs"
  - "crates/openlogi-desktop/src/main.rs"
  - "crates/openlogi-desktop/src/services/i18n.rs"
  - "crates/openlogi-overlay/src/main.rs"
  - ".config/crowdin.yml"
  - ".github/scripts/i18n/**"
  - ".github/workflows/crowdin.yml"
---

# i18n (rust-i18n + Crowdin)

- **`locales/en.yml` is the English source of truth** (the English text IS the
  key). When adding or changing copy, add the **same key to every**
  `locales/<code>.yml` in the same change, at the **same position** — position is
  a convention (it keeps all catalogs diff-aligned), the key set is what the
  parity test enforces.
- Values for non-English locales may be a best-effort translation (or temporarily
  match English when no better string is ready). Crowdin improves them later;
  do not leave keys missing — the parity test fails the build.
- YAML is parsed at **compile time** — a syntax error fails the entire GUI
  build. Quote values containing `: ` or a trailing `:`.
- The locale key test (`openlogi-ui/src/catalog_parity.rs`,
  `locale_files_have_the_same_keys`) requires every non-English file to carry
  **exactly** the keys in `en.yml` (no lag, no extras; key order is not checked).
- Adding a locale: drop the `.yml` in `crates/openlogi-ui/locales/` and add the
  `SUPPORTED` entry in `openlogi-core`'s `locale.rs` (picker order: native-name
  alphabetical per script). The parity test's `include_str!` list must grow too —
  it asserts its own coverage against `SUPPORTED`, so forgetting fails the build
  rather than silently leaving a catalog unchecked.
- Check with `cargo test -p openlogi-ui locale` (catalog parity, runs everywhere)
  and `cargo test -p openlogi-desktop i18n` (end-to-end key resolution; CI's Linux
  tests exclude the app crate, so macOS CI owns that coverage; a local macOS run needs
  the Xcode/Metal env from devenv). The overlay and agent each pin one non-English key
  in their `the_shared_catalog_is_wired_up` test, so a broken relative `i18n!` path
  fails loudly instead of silently serving English.
- Crowdin workflow (`.config/crowdin.yml` + `.github/workflows/crowdin.yml`):
  1. Snapshot complete `locales/*.yml`
  2. Upload `en.yml` sources + per-language translations from git
  3. Download with `skip_untranslated_strings` (sparse export is OK)
  4. **Merge** via `.github/scripts/i18n/merge_crowdin_download.py` — only apply values
     that differ from English; never delete keys; never keep English fill-in
  5. Open `crowdin/i18n` only when a real value improved
