...
Linux CI includes `openlogi-ui`, so catalog parity runs there. It excludes
`openlogi-desktop`, so the desktop catalog-wiring and key-resolution tests run only on
macOS CI. The focused i18n plan runs both owners in order:
`cargo xtask ci i18n`.
...
| `crates/openlogi-ui/locales/**` or catalog wiring | `cargo test -p openlogi-ui locale` (portable parity), then `cargo test -p openlogi-desktop i18n` (wiring/resolution; Linux CI omits this crate) |
