...
Before calling a permission path known-broken or fixed, query the current issue
tracker and the latest release notes. Read the matching report, comments, current
code, and logs. Treat the report's proposed diagnosis as a claim until the
provenance and direct evidence in §§1–4 establish the root cause.
