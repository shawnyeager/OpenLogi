...
Panels gate on measured or last-good `Capabilities`; the only kind-derived fallback is
the centralized never-probed, currently-offline exception in `.claude/rules/gui.md`.
Commits go through `AppState`, never straight to `Config`.
